...
public class CidadesImpl implements CidadesQueries {
	...
	public Page<Cidade> filtrar(CidadesFilter filtro, Pageable pageable) {
	...
	private void adicionarFiltro(CidadesFilter filtro, Criteria criteria) {
		if (filtro != null) {
	...
	private Long total(CidadesFilter filtro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cidade.class);
		adicionarFiltro(filtro, criteria);
	...
	private boolean isEstadoPresente(CidadesFilter filtro) {
